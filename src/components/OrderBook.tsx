"use client";

import { useEffect, useState } from 'react';

interface OrderLine {
  price: number;
  size: number;
  total: number;
  type: 'ask' | 'bid';
}

export default function OrderBook({ currentPrice }: { currentPrice: string | null }) {
  const [asks, setAsks] = useState<OrderLine[]>([]);
  const [bids, setBids] = useState<OrderLine[]>([]);
  const [maxTotal, setMaxTotal] = useState(1);

  useEffect(() => {
    const fetchDepth = async () => {
      try {
        const res = await fetch('/api/binance/depth?symbol=SOLUSDT&limit=15');
        const data = await res.json();

        if (data.asks && data.bids) {
          let currentAskTotal = 0;
          const formattedAsks = data.asks.slice(0, 15).reverse().map((ask: string[]) => {
            const size = parseFloat(ask[1]);
            currentAskTotal += size;
            return { price: parseFloat(ask[0]), size, total: currentAskTotal, type: 'ask' };
          });

          let currentBidTotal = 0;
          const formattedBids = data.bids.slice(0, 15).map((bid: string[]) => {
            const size = parseFloat(bid[1]);
            currentBidTotal += size;
            return { price: parseFloat(bid[0]), size, total: currentBidTotal, type: 'bid' };
          });

          setAsks(formattedAsks);
          setBids(formattedBids);
          setMaxTotal(Math.max(currentAskTotal, currentBidTotal));
        }
      } catch (err) {
        console.error("Order book fetch error:", err);
      }
    };

    fetchDepth();
    const interval = setInterval(fetchDepth, 2000); // Update every 2 seconds

    return () => clearInterval(interval);
  }, []);

  return (
    <div className="w-full h-full flex flex-col font-mono text-[10px] sm:text-xs">
      <div className="flex justify-between text-gray-500 mb-2 px-2">
        <span>Fiyat (USD)</span>
        <span>Miktar (SOL)</span>
      </div>
      
      {/* Asks (Sells) */}
      <div className="flex-1 flex flex-col justify-end overflow-hidden">
        {asks.map((ask, i) => (
          <div key={`ask-${i}`} className="flex justify-between px-2 py-0.5 relative group cursor-pointer hover:bg-white/5">
            <div 
              className="absolute right-0 top-0 h-full bg-red-500/10 z-0 transition-all duration-300" 
              style={{ width: `${Math.min(100, (ask.total / maxTotal) * 100)}%` }} 
            />
            <span className="text-red-400 z-10">{ask.price.toFixed(3)}</span>
            <span className="text-gray-300 z-10">{ask.size.toFixed(2)}</span>
          </div>
        ))}
      </div>
      
      {/* Spread / Current Price */}
      <div className="my-2 py-2 border-y border-white/5 text-center bg-black/20">
        <span className="text-lg font-bold text-white tracking-widest">{currentPrice || "---"}</span>
      </div>

      {/* Bids (Buys) */}
      <div className="flex-1 flex flex-col overflow-hidden">
        {bids.map((bid, i) => (
          <div key={`bid-${i}`} className="flex justify-between px-2 py-0.5 relative group cursor-pointer hover:bg-white/5">
            <div 
              className="absolute right-0 top-0 h-full bg-green-500/10 z-0 transition-all duration-300" 
              style={{ width: `${Math.min(100, (bid.total / maxTotal) * 100)}%` }} 
            />
            <span className="text-green-400 z-10">{bid.price.toFixed(3)}</span>
            <span className="text-gray-300 z-10">{bid.size.toFixed(2)}</span>
          </div>
        ))}
      </div>
    </div>
  );
}
