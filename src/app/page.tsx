  ...
  useEffect(() => {
    ...
    const fetchHomeData = async () => {
      try {
        ...
        // Fetch recent wins for the ticker
        ...
        if (!winsError && winsData && winsData.length > 0) {
          ...
          setRecentWins([
            ...
            { username: "vip***er1", game_title: "Sweet Bonanza", win_amount: 8900 },
          ]);
        }
      } catch (err) {
              ...
              <ul className="space-y-2 text-sm text-gray-400 font-medium">
                {providerCounts.map((p, idx) => (
                  <li key={idx} className="flex items-center justify-between p-2 rounded-lg hover:bg-white/5 cursor-pointer hover:text-white transition-colors group">
                    <span className="group-hover:translate-x-1 transition-transform">{p.provider}</span>
                    <span className="text-[10px] bg-white/5 px-2 py-1 rounded text-gray-500 group-hover:bg-[#16a34a]/20 group-hover:text-[#16a34a]">{p.count}</span>
                  </li>
                ))}
            ...
            <div className="flex gap-8 animate-[marquee_20s_linear_infinite] whitespace-nowrap">
              {[...recentWins, ...recentWins].map((win, idx) => (
                <div key={idx} className="flex items-center gap-3 text-sm">
                  <span className="text-gray-400 font-mono text-xs">{win.username}</span>
            ...
            {filteredGames.length === 0 ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 2xl:grid-cols-5 gap-4 md:gap-6">
                {filteredGames.map((game) => (
                  <Link href={`/${game.slug === 'deep-needle' ? 'deep-needle' : `game/${game.slug}`}`} key={game.id} className="casino-card group aspect-[3/4] block cursor-pointer">
                    {game.is_hot && (
